...
if (online) {
    ...
    config.plugins = [
        ...
        new HtmlwebpackPlugin({
            filename: 'app/views/Ddtalk/Index.phtml',
            chunks: ['app', 'lib'],
            template: path.join(__dirname, '../src/pages/app.html')
        }),
        new webpack.optimize.CommonsChunkPlugin('lib', 'public/static/ddtalk/js/lib.[hash].js'),
        new ExtractTextPlugin('public/static/ddtalk/css/[name].[hash].css'),
        new ReplaceText()
    ];
} else {
    // 设置静态资源 domain
...
ReplaceText.prototype.apply = function(compiler) {
    compiler.plugin('done', function(stats) {
        var fileName = 'app/views/Ddtalk/Index.phtml';
